...
public:
	...
	//成员变量
	std::vector<StationInfoIP>StationIPArray;//工位的全部摄像头IP
	std::vector< CarInfoIP>CarIPArray;//行车的五个摄像头IP
	std::vector<std::pair<int, double> >tagPosition;//标签号和标签位置
	char* carNum;            //行车号
	std::string globalIP;   //行车的全景摄像头IP
	//数据库操作对象
